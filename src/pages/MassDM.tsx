...
import { DialogModel, MessageModel, UserModel } from "../../shared/types";
import {
  Box,
  Button,
  Divider,
...
const MassDM = () => {
  ...
  const getUsersFromUsernames = (usernames: string[]) => {
    window.Main.sendAsyncRequest({
      method: "getUsersFromUsernames",
      params: [usernames],
    });
    window.Main.once(CHANNEL_USERS, (data: UserModel[]) => {
      if (!data) {
  ...
  const waitForMsgSent = async (timeout: number, id: string) => {
  ...
  const handleListUsernamesUpload = (e) => {
    ...
    Papa.parse(e.target.files[0], {
      ...
      complete: async (results) => {
        ...
        // The input CSV can be anything but must contain a usrname column
        const usernames = results.data
          .filter(
            (row: { username: string; [key: string]: any }) =>
              row.username && row.username.trim() !== "",
          )
          .map((row: { username: string; [key: string]: any }) => row.username);

        await getUsersFromUsernames(usernames);
      },
    });
  };
  ...
  async function processGroups(groups: DialogModel[]) {
    const _participants = [];
