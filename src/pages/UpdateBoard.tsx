...
export const UpdateBoard = () => {
  ...
  useEffect(() => {
    ...
    const startTelegram = () => {
      setRunning(true)
      window.Main.sendAsyncRequest({method: 'startBoardUpdates', params: [userToAdd]});
  ...
  return (
    <Flex justifyContent={'center'} height={'100vh-40px'}>
      ...
      {!allBoards && <><Spinner/><p>Loading your Monday boards. Taking too long ? Retry in a minute or reset your Monday
        configuration</p></>}
        ...
        <form onSubmit={handleSubmit}>
          <Box
            ...
            {selectedBoards.map((board, index) => {
              return (
                <Flex flexDir={"column"}>
                  <Flex justifyContent={'center'} alignItems={'center'} key={`${board.name}-${index}`} margin={'10px'}>
                    <Select defaultValue={board.id}
                            onChange={(e) => onBoardChange(e, index)}>
                      {allBoards.map((board) => {
                        return (
                          <option key={board.name} value={board.id}>{board.name}</option>
                        )
                      })}
                    </Select>
                    <DeleteIcon marginLeft={'4px'} onClick={() => deleteBoard(index)}></DeleteIcon>
                  </Flex>
              ...
              {userToAdd.map((id) => {
                ...
                return (
                  <Box>
                    ...
                    <DeleteIcon marginLeft={'4px'} onClick={(e) => {
